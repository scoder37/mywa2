...
let ibg = 'https://telegra.ph/file/ebb5a84cf76c058f95f67.jpg'
            let pp = 'https://telegra.ph/file/7a500b34519811d5c55dc.jpg'
            let ppgc = 'https://telegra.ph/file/aa86bc1f04960c0d7a8f4.jpg'
let bg = fs.readFileSync('tmp/l3.jpg')
            try {
              pp = await uploadImage(await (await fetch(await this.getProfilePicture(user))).buffer())
              ppgc = await uploadImage(await (await fetch(await this.getProfilePicture(jid))).buffer())
            } catch (e) {
            } finally {
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat datang, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '').replace('@mem', groupMetadata.participants.length) :
                (chat.sBye || this.bye || conn.bye || 'Sampai jumpa, @user!')).replace(/@user/g, '@' + user.split`@`[0])
 
// NEW
let neww = `https://hardianto-chan.herokuapp.com/api/welcome3?profile=${pp}&name=${this.getName(user)}&bg=${ibg}&namegb=${this.getName(jid)}&member=${groupMetadata.participants.length}`
let hwel = await (await fetch(neww)).buffer()
...
conn.sendBtnImg(jid, action === 'add' ? hwel : hle, text, global.footer, 'LIST MENU', '#menu', false, {
      ...
      case 'demote':
	    ...
	    let banp = fs.readFileSync('tmp/l3.jpg')
        if (chat.detect) this.send2Loc(jid, banp, text, global.footer, 'MENU', '#menu', 'OWNER', '#owner', null, {
          contextInfo: {
...
await this.send2Loc(from, banc, `*Anti Call System*

${a}Halo, @${from.split`@`[0]}  👋
kamu telah melanggar peraturan bot yaitu menelfon bot.
jika kamu menelfon lebih dari 3 kali, maka nomor kamu akan diblok otomatis

${user.call}/3${a}`, global.footer, 'LIST MENU', '#menu', 'RULES', '#rules', null, { contextInfo: {
        mentionedJid: [from]
      }
    })
    if (user.call == 3) {
